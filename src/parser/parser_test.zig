...
test "if_expression" {
    ...
    const statement: *ast.Statement = @ptrCast(@alignCast(program.statements[0].ptr));
    const statement_string = try statement.string(test_allocator);
    defer test_allocator.free(statement_string);
    std.debug.print("The statement is {s}\n", .{statement_string});
}
